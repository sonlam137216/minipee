...
The current schema is defined in `backend/migrations/000001_init.up.sql`.
...
| `products` | `backend/internal/products` | `products.PostgresRepository` | Draft product creation writes through `Create`. |

There are no current tables for buyers, orders, payments, inventory, shipping, carts, product publishing, or AI features.
...
- `products.PostgresRepository.FindSellerDraftByID` reads one draft product where `id = $1 AND seller_id = $2 AND status = 'draft'`.
