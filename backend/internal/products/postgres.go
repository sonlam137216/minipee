...
func (r *PostgresRepository) FindBySellerID(ctx context.Context, sellerID string, productID string) (Product, error) {
	var product Product
...
func (r *PostgresRepository) PublishDraftBySeller(ctx context.Context, sellerID string, productID string, now time.Time) (Product, error) {
	var product Product
	err := r.db.QueryRow(ctx, `
		UPDATE products
		SET status = 'published', updated_at = $3
		WHERE id = $1 AND seller_id = $2 AND status = 'draft'
		RETURNING id, seller_id, name, description, status, created_at, updated_at
	`, productID, sellerID, now.UTC()).Scan(
		&product.ID,
		&product.SellerID,
		&product.Name,
		&product.Description,
...
func (r *PostgresRepository) FindPublishedByID(ctx context.Context, productID string) (Product, error) {
	var product Product
	err := r.db.QueryRow(ctx, `
		SELECT id, seller_id, name, description, status, created_at, updated_at
		FROM products
